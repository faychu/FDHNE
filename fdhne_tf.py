...
h_c_drop = tf.nn.dropout(h_c, keep_prob)
W = weight_variable([40, 18])
b = bias_variable([18])
# y = tf.matmul(h_c_drop, W) + b
y = tf.nn.softmax(tf.matmul(h_c_drop, W) + b)  # 使用softmax作为多分类激活函数，在Ls中体现
...
joint_loss = Ls + L1st_loss
# optimizers
Optimizer = tf.train.GradientDescentOptimizer(0.0001).minimize(joint_loss)
# accuracy
